...
func openStatReadDemo() {
	...
	if err != nil {
		fmt.Println("open: could not open file")
		return
	} else {
	...
	stat, err := file.Stat()
	if err != nil {
		fmt.Println("Stat: error getting file info")
		return
	}
	...
	_, err = file.Read(buf)
	if err != nil {
		fmt.Println("Read: could not read file")
		return
	}
...
func readFileDemo() {
	...
	if err != nil {
		fmt.Println("ReadFile: could not read file")
		return
	}
...
func createDemo() {
	...
	if err != nil {
		fmt.Println("Create: could not create file")
		return
	} else {
		// ensure close
		defer file.Close()
